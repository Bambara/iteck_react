                                            ...
                                            index <= 3 && (
                                                <div className={`accordion-item ${index === 3 ? '' : 'border-bottom'} rounded-0`} key={index}>
                                                    <h2 className="accordion-header" id={`heading${item.id + 10}`}>
                                                        <button className="accordion-button collapsed rounded-0 py-4" type="button" data-bs-toggle="collapse"
                                                                data-bs-target={`#collapse${item.id + 10}`} aria-expanded="true" aria-controls={`collapse${item.id + 10}`}>
                                                            {item.question}
                                                        </button>
                                                    </h2>
                                                    <div id={`collapse${item.id + 10}`} className="accordion-collapse collapse rounded-0" aria-labelledby={`heading${item.id + 10}`}
                                        ...
                                        data.map((item, index) => (
                                            index > 3 && (
                                                <div className={`accordion-item ${index === faq.length - 1 ? '' : 'border-bottom'} rounded-0`} key={index}>
                                                    <h2 className="accordion-header" id={`heading${item.id + 10}`}>
                                                        <button className={`accordion-button ${index !== 4 ? 'collapsed' : ''} rounded-0 py-4`} type="button" data-bs-toggle="collapse"
                                                                data-bs-target={`#collapse${item.id + 10}`} aria-expanded="true" aria-controls={`collapse${item.id + 10}`}>
                                                            {item.question}
                                                        </button>
                                                    </h2>
                                                    <div id={`collapse${item.id + 10}`} className={`accordion-collapse collapse rounded-0 ${index !== 4 ? '' : 'show'}`}
                                                         aria-labelledby={`heading${item.id + 10}`} data-bs-parent="#accordionSt4">
                                                        <div className="accordion-body">
                                                            {
                                                                rtl ? item.answer : <>{item.answer.part1} <br/> {item.answer.part2}</>
                                                            }
