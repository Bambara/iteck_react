                                        ...
                                        features.map((feature, index) => (
                                            <div className="accordion-item" key={index}>
                                                <h2 className="accordion-header" id="flush-headingOne">
                                                    <button className="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target={`#flush-collapse${feature.id}`}
                                                            aria-expanded="false" aria-controls={`flush-collapse${feature.id}`}>
                                                        <span> {feature.id}. </span> {feature.title}
                                                    </button>
                                                </h2>
                                                <div id={`flush-collapse${feature.id}`} className={`accordion-collapse collapse ${index === 1 ? 'show' : ''}`} aria-labelledby="flush-headingOne"
