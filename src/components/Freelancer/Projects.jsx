                    ...
                    <ul className="nav nav-pills mb-40 justify-content-center" id="pills-tab" role="tablist">
                        {
                            projectsData.linkTabs.map((link, idx) => (
                                <li className="nav-item" role="presentation" key={idx}>
                                    <button className={`nav-link ${idx === 0 ? 'active' : ''}`} id={`pills-${link.id}-tab`} data-bs-toggle="pill" data-bs-target={`#pills-${link.tab}`}
                                            type="button" role="tab" aria-controls={link.control} aria-selected={idx === 0 ? 'true' : 'false'}>{link.name}</button>
                                </li>
                            ))
                        }
                    </ul>
